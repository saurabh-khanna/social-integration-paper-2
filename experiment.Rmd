...
lm1 <-
  seg_scores %>% 
  lm_robust(b_seg_new ~ female + age + ses + hs_private + mother_college + father_college, data = ., fixed_effects = ~ classid, clusters = classid, se_type = "stata")
...
lm1 <-
  ...
  lm_robust(e_ct_score_perc ~ b_seg_new + female + age + ses + hs_private + mother_college + father_college, data = ., fixed_effects = ~ classid, clusters = classid, se_type = "stata")
...
lm7 <-
  seg_scores %>%
  filter(reservation == 1) %>% 
  lm_robust(e_math_g3_score_perc ~ b_seg_new + female + age + ses + hs_private + mother_college + father_college, data = ., fixed_effects = ~ classid, clusters = classid, se_type = "stata")
...
lm9 <-
  seg_scores %>%
  filter(reservation == 1) %>% 
  lm_robust(e_physics_g3_score_perc ~ b_seg_new + female + age + ses + hs_private + mother_college + father_college, data = ., fixed_effects = ~ classid, clusters = classid, se_type = "stata")
...
knitreg(list(lm1, lm2, lm7, lm8, lm9, lm10), custom.note = "%stars \n All models control for department fixed effects. \n Standard errors clustered at department level.", stars = c(0.01, 0.05, 0.1), include.ci = FALSE, custom.model.names = c("CT-Res", "CT-NRes", "Math-Res", "Math-NRes", "Physics-Res", "Physics-NRes"))
```
...
seg_scores %>% 
  ...
  ggplot(aes(b_seg_new, e_ct_score_perc)) +
